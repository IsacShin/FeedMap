...
// MARK: - ADMOB 관련
let BANNER_ADMOBKEY             = "ca-app-pub-6912457818283583/6384622245"
...
final class CommonAdManager: NSObject {
    ...
    public func addBanner(parentVC: BaseVC, subV: UIView) {
        ...
        self.bannerView.do {
            $0.adUnitID = TEST_BANNER_ADMOBKEY
    ...
    public func loadFullAd(parentVC: BaseVC) {
        DispatchQueue.main.async {
            GADInterstitialAd.load(withAdUnitID: TEST_FULL_ADMOBKEY, request: GADRequest()) { ad, error in
                if error != nil { return }
                if let ad = ad {
